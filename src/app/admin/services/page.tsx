...
import { Plus, Pencil, Trash2, Check, X, Star, Loader2, Eye } from 'lucide-react';
...
export default function ServicesPage() {
  ...
  const startEdit = (s: Service) => {
    setEditId(s.id);
    setEditData({ name: s.name, details: s.details, price: s.price, is_bestseller: s.is_bestseller, is_featured: s.is_featured, featured_image: s.featured_image ?? '', featured_description: s.featured_description ?? '', featured_duration: s.featured_duration ?? '' });
  };
  ...
  return (
    <div className="space-y-5 max-w-5xl mx-auto">
      ...
      <div className="flex flex-wrap items-center gap-4 px-1 text-xs text-zinc-500 dark:text-zinc-400">
        ...
        <span className="flex items-center gap-1.5"><Eye size={13} className="text-blue-400" /> Featured = tampil di bagian atas landing page</span>
      </div>
      ...
      <div className="flex gap-1 bg-zinc-100 dark:bg-zinc-800 p-1 rounded-lg overflow-x-auto">
        {CATEGORIES.map(c => (
          <button
            ...
            className={`px-3 py-1.5 rounded-md text-xs font-medium whitespace-nowrap transition-colors ${activeTab === c.id ? 'bg-white dark:bg-zinc-700 text-zinc-900 dark:text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300'}`}
          >
              ...
              {editId === svc.id ? (
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <input type="number" className="admin-input font-mono" value={editData.price ?? 0} onChange={e => setEditData(d => ({ ...d, price: Number(e.target.value) }))} />
                    </div>
                  </div>
                  <div>
                    <label className="text-xs font-medium text-zinc-500 mb-1 block">Detail / Deskripsi</label>
                    <textarea className="admin-input resize-none" rows={2} value={editData.details ?? ''} onChange={e => setEditData(d => ({ ...d, details: e.target.value }))} />
                  </div>
                  {editData.is_featured && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-3 bg-blue-50 dark:bg-blue-950/20 rounded-lg border border-blue-100 dark:border-blue-900">
                      <div>
                        ...
                        <input className="admin-input text-xs" value={editData.featured_description ?? ''} onChange={e => setEditData(d => ({ ...d, featured_description: e.target.value }))} />
                      </div>
                      <div>
                        <label className="text-xs font-medium text-blue-600 mb-1 block">Featured Duration (e.g. 145 Min)</label>
                        <input className="admin-input text-xs" value={editData.featured_duration ?? ''} onChange={e => setEditData(d => ({ ...d, featured_duration: e.target.value }))} />
                      </div>
                      <div className="sm:col-span-2">
                        <label className="text-xs font-medium text-blue-600 mb-1 block">Featured Image Path (e.g. /featured-refine.png)</label>
                        <input className="admin-input text-xs font-mono" value={editData.featured_image ?? ''} onChange={e => setEditData(d => ({ ...d, featured_image: e.target.value }))} />
                      </div>
                    </div>
                  )}
                  <div className="flex gap-4">
                    <label className="flex items-center gap-2 cursor-pointer text-sm text-zinc-600 dark:text-zinc-400">
                      <input type="checkbox" checked={editData.is_bestseller ?? false} onChange={e => setEditData(d => ({ ...d, is_bestseller: e.target.checked }))} className="accent-amber-500" />
                      Best Seller
                    </label>
                    <label className="flex items-center gap-2 cursor-pointer text-sm text-zinc-600 dark:text-zinc-400">
                      <input type="checkbox" checked={editData.is_featured ?? false} onChange={e => setEditData(d => ({ ...d, is_featured: e.target.checked }))} className="accent-blue-500" />
                ...
                <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {svc.is_bestseller && <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded bg-amber-50 text-amber-600 dark:bg-amber-950/40 dark:text-amber-400 border border-amber-200 dark:border-amber-800">Best Seller</span>}
                        {svc.is_featured && <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded bg-blue-50 text-blue-600 dark:bg-blue-950/40 dark:text-blue-400 border border-blue-200 dark:border-blue-800">Featured</span>}
                      </div>
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    <button onClick={() => startEdit(svc)} className="p-2 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-400 hover:text-zinc-600"><Pencil size={14} /></button>
                    <button onClick={() => deleteService(svc.id)} className="p-2 rounded-lg hover:bg-red-50 dark:hover:bg-red-950/30 text-zinc-400 hover:text-red-500"><Trash2 size={14} /></button>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-6">
          ...
          <div className="relative w-full sm:max-w-md bg-white dark:bg-zinc-900 rounded-t-2xl sm:rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-2xl p-6 space-y-3 z-10 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-1">
              ...
              <button onClick={() => setShowAdd(false)} className="p-1.5 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-400"><X size={16} /></button>
            </div>
            <input className="admin-input" placeholder="Nama Layanan" value={newSvc.name} onChange={e => setNewSvc(n => ({ ...n, name: e.target.value }))} />
            <textarea className="admin-input resize-none" rows={2} placeholder="Detail / Deskripsi" value={newSvc.details} onChange={e => setNewSvc(n => ({ ...n, details: e.target.value }))} />
            ...
            <div className="flex gap-4">
              <label className="flex items-center gap-2 cursor-pointer text-sm text-zinc-600 dark:text-zinc-400">
                <input type="checkbox" checked={newSvc.is_bestseller} onChange={e => setNewSvc(n => ({ ...n, is_bestseller: e.target.checked }))} className="accent-amber-500" /> Best Seller
              </label>
              <label className="flex items-center gap-2 cursor-pointer text-sm text-zinc-600 dark:text-zinc-400">
                <input type="checkbox" checked={newSvc.is_featured} onChange={e => setNewSvc(n => ({ ...n, is_featured: e.target.checked }))} className="accent-blue-500" /> Featured
              </label>
            </div>
            {newSvc.is_featured && (
              <div className="space-y-2 p-3 bg-blue-50 dark:bg-blue-950/20 rounded-lg">
                <input className="admin-input text-xs" placeholder="Featured description" value={newSvc.featured_description} onChange={e => setNewSvc(n => ({ ...n, featured_description: e.target.value }))} />
                <input className="admin-input text-xs" placeholder="Duration (e.g. 145 Min)" value={newSvc.featured_duration} onChange={e => setNewSvc(n => ({ ...n, featured_duration: e.target.value }))} />
                <input className="admin-input text-xs font-mono" placeholder="Image path (e.g. /featured-refine.png)" value={newSvc.featured_image} onChange={e => setNewSvc(n => ({ ...n, featured_image: e.target.value }))} />
              </div>
            )}
